...
public class AppDelegate implements App, AppLifecycles {
    ...
    protected Application.ActivityLifecycleCallbacks activityLifecycleForRxLifecycle;

    private List<ConfigModule> configModuleList; //各个模块的配置

    private List<AppLifecycles> mAppLifecycles = new ArrayList<>(); //各个模块的应用生命周期
    private List<Application.ActivityLifecycleCallbacks> mActivityLifecycles = new ArrayList<>();
    private ComponentCallbacks2 mComponentCallback;

    public AppDelegate(@NonNull Context context) {
        //用反射, 将 AndroidManifest.xml 中带有 ConfigModule 标签的 class 转成对象集合（List<ConfigModule>）
        this.configModuleList = new ManifestParser(context).parse();

        //遍历之前获得的集合, 执行每一个 ConfigModule 实现类的某些方法
        for (ConfigModule module : configModuleList) {
...
//                .globalConfigModule(getGlobalConfigModule(mApplication, mModules))//全局配置
        ...
        mComponentCallback = new AppComponentCallbacks(application, appComponent);
...
//        return builder.build();
     ...
     *
     * @return AppComponent
     * @see #(Context) 可直接获取 {@link AppComponent}
     */
    @NonNull
    @Override
...
//                        ? Application.class.getName() : application.getClass().getName());
     ...
     * {@link Application}、{@link Activity}、{@link Service}、{@link ContentProvider}、{@link } 实现了 {@link ComponentCallbacks2} 接口
